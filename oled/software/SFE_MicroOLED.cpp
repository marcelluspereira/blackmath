...
Modified by Marcellus Pereira <marcellus@laboratoriosacme.net>
September 23, 2018
>Prime objective: run on Black Math board
>diff: The OLED display is named "Ganzts 128x32", using SSD1306 controller
http: ....
...
static uint8_t screenmemory [512] = { //BlackMath logo
...
void MicroOLED::begin()
{
	...
	command(SETSTARTLINE | 0x40);	// line #0

	command(CHARGEPUMP);			// enable charge pump
	command(0x14);

	command(SEGREMAP | 0xa1);
	command(COMSCANDEC);
...
size_t MicroOLED::write(const char* string) {
	//uint8_t	contador = 0;
	// while( (*(string+contador) != 0) && (contador < 255)){
		// write(*(string+contador));
	// }
